...
export default function CategoryPage({ params }: CategoryPageProps) {
  ...
  const category = categories.find(c => {
    const matches = [
      c.slug === params.id,                                                    // electronics-234100 === electronics-234100
      c.id === params.id,                                                      // exact ID match
      c.slug.startsWith(params.id + '-'),                                     // electronics-234100.startsWith('electronics-')
      c.slug.toLowerCase() === params.id.toLowerCase(),                       // case insensitive
      c.name.toLowerCase().replace(/\s+/g, '-') === params.id.toLowerCase(),  // 'Electronics' -> 'electronics'
      c.slug.split('-')[0] === params.id,                                     // 'electronics-234100'.split('-')[0] === 'electronics'
      c.slug.split('-')[0].toLowerCase() === params.id.toLowerCase()          // case insensitive version
    ];
    return matches.some(Boolean);
  });
  
  // Call useProducts hook BEFORE any conditional returns
  // Use the found category ID if available, otherwise use params.id (slug)
  const categoryParam = category?.id || params.id;
